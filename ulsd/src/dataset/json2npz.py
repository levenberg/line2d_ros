...
def json2npz(src_path, dst_path, split, cfg, plot=False):
    ...
    json_file = os.path.join(src_path, '{}.json'.format(split))
    try:
    ...
    def call_back(data):
        ...
        if split == 'train':
            for i in range(len(tfs)):
                ...
                prefix = os.path.join(dst_path, split, filename.split('.')[0] + '_{}'.format(i))
            ...
            save_npz('{}'.format(prefix), image, lines.copy(), centers, cfg)
