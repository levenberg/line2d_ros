...
def save_lines(image, lines, heatmap_size, filename, plot=False):
...
def test(model, loader, cfg, device):
    ...
    if cfg.evaluate:
        ...
        print('mAPJ: {:.1f} | P: {:.1f} | R: {:.1f}'.format(mAPJ, P, R))
        msAP, P, R, sAP = eval_sAP(cfg.groundtruth_path, cfg.output_path)
        print('msAP: {:.1f} | P: {:.1f} | R: {:.1f} | sAP5: {:.1f} | sAP10: {:.1f} | sAP15: {:.1f}'.format(msAP, P, R, sAP[0], sAP[1], sAP[2]))
...
if __name__ == '__main__':
    ...
    device = torch.device('cuda:{}'.format(cfg.gpu) if use_gpu else 'cpu')
