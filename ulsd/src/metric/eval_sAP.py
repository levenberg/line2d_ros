...
def eval_sAP(gt_path, pred_path, save_path=None, order=None):
    ...
    if save_path is not None:
        ...
        figure.savefig(os.path.join(save_path, 'sAP10-{}.pdf'.format(order)), format='pdf', bbox_inches='tight')
        sio.savemat(os.path.join(save_path, 'sAP10-ULSD-{}.mat'.format(order)), {'rcs': rcs, 'prs': prs, 'AP': sAP10})
        plt.show()
