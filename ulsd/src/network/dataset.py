...
class Dataset(Data.Dataset):
    ...
    def __getitem__(self, index):
        if self.with_label:
            ...
            with np.load(label_file) as npz:
                label = {name: npz[name] for name in ['jmap', 'joff', 'cmap', 'coff', 'lvec', 'lmap',
                                                      'junc', 'lpos', 'lneg']}
    ...
    def transforms(self, image, label=None):
        if self.with_label:
            ...
            lvec = torch.from_numpy(label['lvec']).float()
            ...
            map = {'jmap': jmap, 'joff': joff, 'cmap': cmap, 'coff': coff, 'lvec': lvec, 'lmap': lmap}
